...
/**
 * @ngdoc service
 * @name ngCordovaMocks.cordovaPushV5
 *
 * @description
 * A service for testing push notifications features
 * in an app build with ngCordova.
...
ngCordovaMocks.factory('$cordovaPushV5', ['$q', '$timeout', '$rootScope', function ($q, $timeout, $rootScope) {
  ...
  return {
     ...
     * @ngdoc property
     * @name throwsError
     * @propertyOf ngCordovaMocks.cordovaPushV5
     *
     * @description
     * A flag that signals whether a promise should be rejected or not.
     * This property should only be used in automated tests.
    ...
    initialize: function (config) {
      ...
      if (this.throwsError) {
        defer.reject('There was an initialize error.');
      } else {
        defer.resolve();//technically this should be returning the current push instance
      }
      return defer.promise;
    },
    ...
    unregister: function (options) {
      var defer = $q.defer();
      if (this.throwsError) {
        defer.reject('There was an unregister error.');
      } else {
        defer.resolve();
      }
      return defer.promise;
